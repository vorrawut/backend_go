...
import (
	...
	"safebsc/features/my_farm/models"
...
func (u Controller) GetMyFarm(c *gin.Context) {
	getMyFarm(c)
}

func getMyFarm(c *gin.Context) {
	log.Println(">>>>>>>>>>>>   getMyFarm")
...
func (dbClient connectionClient) findMyFarms(walletAddress string, supportedFarms []shared.SupportedFarm) ([]models.MyFarm, error) {
...
func (dbClient connectionClient) findUserFarmsByWallet(walletAddress string) ([]models.UserPortFolio, error) {
	collection := dbClient.db.Client.Database(dbClient.db.DatabaseName).Collection("user_portfolio_latests")

	snaps := make([]models.UserPortFolio, 0)
...
func (dbClient connectionClient) findSnapShortByWallet(walletAddress string) ([]models.UserPortFolio, error) {
	collection := dbClient.db.Client.Database(dbClient.db.DatabaseName).Collection("user_portfolios")

	var lastSnapshot models.UserPortFolio
	snaps := make([]models.UserPortFolio, 0)
...
func transformToFarms(walletAddress string, snaps []models.UserPortFolio,
	supportedFarms []shared.SupportedFarm,
	tokensRate []models.TokenPrice,
	farmsApr []models.FarmApr) ([]models.MyFarm, error) {
	myFarms := make([]models.MyFarm, 0)
	for _, supportedFarm := range supportedFarms {
		farmSnaps := filterByFarmID(snaps, supportedFarm.OrderID)
		if len(farmSnaps) > 0 {
			farmDetails := make([]models.FarmDetail, 0)
			...
			for _, fs := range farmSnaps {
				...
				farmDetails = append(farmDetails, models.FarmDetail{
			...
			myFarms = append(myFarms, models.MyFarm{
...
func getRewardAndBalance(fs models.UserPortFolio, supportedFarm shared.SupportedFarm, tokensRate []models.TokenPrice, tokens []models.Token) (float64, float64, []models.RewardResponse, error) {
	rewards := make([]models.RewardResponse, 0)
	var reward float64
	...
	for _, rw := range fs.Reward {
		...
		rewards = append(rewards, models.RewardResponse{
			Symbol:       rw.Symbol,
			Reward:       reward,
...
func getTokenPrice(dbClient connectionClient) ([]models.TokenPrice, error) {
	conf := make([]models.TokenPrice, 0)
...
func getFarmApr(dbClient connectionClient) ([]models.FarmApr, error) {
	conf := make([]models.FarmApr, 0)
...
func sumTotalPriceOfFarm(dts []models.FarmDetail) float64 {
	var sum float64
	for _, dt := range dts {
...
func sumTotalPriceOfDetail(rewardUsd float64, tks []models.Token) float64 {
...
func sumTotalBalanceOfDetail(reward float64, tks []models.Token) float64 {
...
func filterByFarmID(ss []models.UserPortFolio, farmID string) (ret []models.UserPortFolio) {
	for _, s := range ss {
		if s.FarmID == farmID {
			ret = append(ret, s)
		}
...
func getRateBySymbol(symbol string, tkp []models.TokenPrice) float64 {
	for _, tk := range tkp {
		if tk.Symbol == symbol {
...
func getRateStrBySymbol(symbol string, tkp []models.TokenPrice) string {
	for _, tk := range tkp {
		if tk.Symbol == symbol {
...
func getRateByTokenAddress(tokenAddress string, tkp []models.TokenPrice) float64 {
	for _, tk := range tkp {
...
func getAprByLpToken(lpToken string, farmsApr []models.FarmApr) models.FarmApr {
	...
	return models.FarmApr{}
}

func getAprByNameAndPoolID(farmName string, poolId int, farmsApr []models.FarmApr) models.FarmApr {
	for _, tk := range farmsApr {
