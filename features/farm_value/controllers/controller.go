...
import (
	...
	"safebsc/features/farm_value/models"
	"safebsc/mongo"
...
type FarmValueController struct{}

func (u FarmValueController) GetFarmValue(c *gin.Context) {
	getFarmValue(c)
}
...
func sumValuePerFarm(farmId string, portValue string, farms []models.Farm) ([]models.Farm, float64) {
...
func sumValueAtTime(userPorts []models.UserPortfolios) []models.DataFarmValue {
	var result []models.DataFarmValue
	for _, userPort := range userPorts {
		...
		if len(result) > 0 && result[len(result)-1].Date == date {
			...
			result = append(result, models.DataFarmValue{
				Date:  date,
				Value: value,
				Farms: farms,
			})
...
func avgValuePerFarm(farms []models.Farm, farms2 []models.Farm) ([]models.Farm, float64) {
...
func sumValuePerDay(dataFarmValues []models.DataFarmValue) []models.DataFarmValue {
	var result []models.DataFarmValue
...
func calculateTotalPercent(dataFarmValue []models.DataFarmValueResponse) float64 {
...
func prepareData(totalValue float64, dataFarmValues []models.DataFarmValue) models.FarmValueResponse {
	sort.SliceStable(dataFarmValues, func(i, j int) bool {
		return dataFarmValues[i].Date.Before(dataFarmValues[j].Date)
	})
	var data []models.DataFarmValueResponse
	for _, dataFarmValue := range dataFarmValues {
		data = append(data, models.DataFarmValueResponse{
			Date:  dataFarmValue.Date,
			Value: fmt.Sprintf("%.4f", dataFarmValue.Value),
		})
	}
	return models.FarmValueResponse{
