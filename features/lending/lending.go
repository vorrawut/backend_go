...
func transformMongoAssetToVenus(mgas []MongoAsset, timestamp float64) (vp VenusPort) {
	...
	for _, mga := range mgas {
		...
		if mga.Type == "Deposit" {
			vp.Deposit = append(vp.Deposit, Asset{
				Address: mga.Address,
				Name:    mga.Name,
				Symbol:  mga.Symbol,
				Amount:  amount,
		...
		if mga.Type == "Borrow" {
			vp.Borrow = append(vp.Borrow, Asset{
				Address: mga.Address,
				Name:    mga.Name,
				Symbol:  mga.Symbol,
				Amount:  amount,
...
func findAssets(ctx context.Context, db *mongodriver.Database, address string, latest float64) []MongoAsset {
	...
	var mongoAssets = make([]MongoAsset, 0)
...
func getReward(address string, rwch chan Reward) {
