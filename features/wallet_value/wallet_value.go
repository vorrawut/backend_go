...
func sumValueAtTime(userWallets []UserWallets) []DataWalletValue {
	var result []DataWalletValue
	for _, userPort := range userWallets {
		...
		if len(result) > 0 && result[len(result)-1].Date == date {
			...
			result = append(result, DataWalletValue{
				Date:       date,
				SumOfValue: value,
			})
		}
...
func sumValuePerDay(dataWalletValues []DataWalletValue) ([]DataWalletValue, error) {
	var result []DataWalletValue
...
func calculageTotalPercent(dataWalletValue []DataWalletValueResponse) float64 {
...
func prepareData(totalValue float64, dataWalletValues []DataWalletValue) WalletValueResponse {
	sort.SliceStable(dataWalletValues, func(i, j int) bool {
		return dataWalletValues[i].Date.Before(dataWalletValues[j].Date)
	})
	var data []DataWalletValueResponse
	for _, dataWalletValue := range dataWalletValues {
		data = append(data, DataWalletValueResponse{
