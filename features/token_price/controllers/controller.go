...
import (
	...
	"safebsc/features/token_price/models"
	"safebsc/mongo"
	"strconv"
	"strings"
	"time"
...
func (u Controller) GetTokenPrice(c *gin.Context) {
	getTokenPrice(c)
}

func filterSupportedTokenByIsHidden(st []models.SupportedToken) []models.SupportedToken {
	result := []models.SupportedToken{}
	for i := range st {
		if !st[i].IsHidden {
...
func calculateChange(tokenPrices []models.TokenPrice) models.Response {
